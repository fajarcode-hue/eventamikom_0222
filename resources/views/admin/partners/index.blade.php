...
<header class="flex justify-between items-center mb-10">
    <div>
        ...
        <p class="text-slate-500 font-medium">Atur mitra kerja sama atau sponsor proyek Anda di sini.</p>
    </div>
...
<div class="mb-6 flex justify-between items-center gap-4">
    <form action="{{ route('admin.partners.index') }}" method="GET" class="w-full max-w-xs relative">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama partner..." class="w-full pl-11 pr-4 py-2.5 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition font-medium text-sm text-slate-700 placeholder-slate-400">
    ...
    @if(request('search'))
        <a href="{{ route('admin.partners.index') }}" class="text-sm font-bold text-rose-600 hover:underline">
            Reset Pencarian
        </a>
    @endif
</div>
        ...
        <table class="w-full text-left border-collapse">
            <thead class="bg-slate-50 text-slate-400 uppercase text-[10px] font-black tracking-widest">
                <tr>
                    ...
                    <th class="px-8 py-4">Nama Perusahaan / Partner</th>
            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-8 py-6">
                        <div class="w-16 h-16 bg-slate-50 rounded-xl p-2 flex items-center justify-center border border-slate-100 shadow-sm">
                    ...
                    <td class="px-8 py-6">
                        <p class="text-sm font-medium text-slate-500">{{ $partner->created_at->format('d M Y') }}</p>
                    </td>
                    <td class="px-8 py-6 flex gap-2 items-center">
                        ...
                        <form action="{{ route('admin.partners.destroy', $partner->id) }}" method="POST" onsubmit="return confirm('Hapus partner ini dari daftar kerja sama?')">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-8 py-10 text-center text-slate-400 font-medium">Data partner tidak ditemukan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
